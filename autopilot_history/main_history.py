...
VOICE = "en-US-AndrewMultilingualNeural"   # warm, natural US narrator (v2 neural)

BANK = json.load(open(os.path.join(HERE, "scripts_history.json"), encoding="utf-8"))
...
def yt_access_token(refresh=None):
    data = json.dumps({"refresh_token": (refresh or os.environ["YT_REFRESH_TOKEN_HISTORY"]).strip()}).encode()
...
def yt_upload(path, meta):
    ...
    body = json.dumps({"snippet": {"title": meta["title"], "description": meta["description"],
                                   "tags": meta.get("tags", []), "categoryId": "27"},
...
CTAS = ["Subscribe for history that explains the world.",
        ...
        "Subscribe — new history every day."]
HISTORY_TAGS = ["history", "historyfacts", "facts", "shorts", "education", "learn",
                "worldhistory", "ancienthistory", "didyouknow", "documentary", "viral", "historical"]

HOOKS = ["Here's what they never taught you.", "History they don't teach in school.",
         "This actually happened.", "You won't believe this is real.", "Wait for this fact."]
